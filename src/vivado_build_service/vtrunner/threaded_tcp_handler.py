...
from .task_factory import task_from_raw_request
from .user_queue import Task, UserQueue
from .config import GeneralConfig
from .streamutil import end_reached
...
class ThreadedTCPHandler(socketserver.BaseRequestHandler):
    ...
    def handle(self):
        ...
        task = task_from_raw_request(
            raw_data, general_config, server_paths.receive_folder
        )
    ...
    def get_request(self, tcp_handler, general_config: GeneralConfig):
        ...
        while True:
            ...
            data += chunk
            if len(chunk) < self.server.general_config.chunk_size or end_reached(
                chunk, self.server.general_config.delimiter
            ):
                break
        return data
    ...
    def process_request(self, data, user, model_number, only_bin) -> Task:  # Server
        task = make_personal_dir_and_get_task(
            user, self.server.server_paths.receive_folder, model_number, only_bin
        )
        task_dir = task.path
